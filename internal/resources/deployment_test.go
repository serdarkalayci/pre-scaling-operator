	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			for _, item := range tt.args.deployment {
				c.PutDeploymentOnGlobalBlackList(item)
			}

			if len(c.ReconcileBlackList) != tt.length {
				t.Errorf("The Blacklist is not the correct length! Got Length = %v, Want Length %v", len(c.ReconcileBlackList), tt.length)
			}
			c.ReconcileBlackList = c.ReconcileBlackList[:0]
		})
	}
...
func TestRemoveFromBlackList(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			c.RemoveDeploymentFromGlobalBlackList(tt.args.deployment[1])
			if len(c.ReconcileBlackList) != tt.length {
				t.Errorf("The Blacklist is not the correct length! Got Length = %v, Want Length %v", len(c.ReconcileBlackList), tt.length)
			}
			if c.ReconcileBlackList[0].Name != tt.expected {
				t.Errorf("The wrong item is leftover on the list! Got Item = %v, Want Item %v", c.ReconcileBlackList[0].Name, tt.expected)
			}
			c.ReconcileBlackList = c.ReconcileBlackList[:0]
		})
	}
...
func TestIsOnBlackList(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			var isOnList = c.IsDeploymentOnBlackList(tt.args.deployment[1])
			if isOnList != tt.expected {
				t.Errorf("The item is not on the list but it should be! Got  = %v, Want  %v", isOnList, tt.expected)
			}
			c.ReconcileBlackList = c.ReconcileBlackList[:0]
		})
	}
